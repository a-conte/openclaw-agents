...
class ImprovedVoiceSkill {
  ...
  async textToSpeech(text, options = {}, playImmediately = false) {
    ...
    // Build the edge-tts command using spawn (safe from shell injection)
    const args = [
      '--text', text,
    ...
    try {
      // Execute the edge-tts command safely without shell
      await new Promise((resolve, reject) => {
        const proc = spawn('edge-tts', args, { stdio: 'pipe' });
        proc.on('close', (code) => code === 0 ? resolve() : reject(new Error(`edge-tts exited with code ${code}`)));
