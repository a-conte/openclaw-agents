...
PROTECTED_FILENAMES = {"SOUL.md", "IDENTITY.md", "USER.md", "PROTOCOL.md"}
...
def protected_file_reason(file_path: str) -> str | None:
    """
    Return a deny reason when a protected OpenClaw identity/protocol file is
    targeted without an explicit bypass.
    """
    if os.getenv("OPENCLAW_ALLOW_PROTECTED_FILES") == "1":
        return None

    path = Path(file_path)
    if path.name in PROTECTED_FILENAMES:
        return (
            f"Permission denied for protected file '{path.name}'. "
            "These files define agent identity/protocol and should only be modified "
            "when explicitly intended. Re-run with OPENCLAW_ALLOW_PROTECTED_FILES=1 "
            "if you truly want to bypass this guardrail."
        )

    return None


def protected_file_access_reason(tool_name: str, tool_input: dict) -> str | None:
    """
    Detect protected file access across direct file tools and bash commands.
    """
    if tool_name in {"Read", "Edit", "MultiEdit", "Write"}:
        file_path = tool_input.get("file_path", "")
        if file_path:
            return protected_file_reason(file_path)
    ...
    if tool_name == "Bash":
        ...
        for name in PROTECTED_FILENAMES:
            ...
            if re.search(pattern, command):
                return protected_file_reason(name)
...
def main():
    try:
        ...
        protected_reason = protected_file_access_reason(tool_name, tool_input)
        if protected_reason:
            response = create_deny_response(protected_reason, interrupt=False)
