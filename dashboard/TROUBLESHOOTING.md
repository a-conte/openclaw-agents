...
```bash
ls -la ~/openclaw-agents/dashboard/data/tasks.json
```
Should be writable by your user. If missing:
```bash
mkdir -p ~/openclaw-agents/dashboard/data
echo '[]' > ~/openclaw-agents/dashboard/data/tasks.json
```
...
- **Working dir:** `~/openclaw-agents/dashboard/`
- **Port:** 3000
